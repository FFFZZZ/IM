...
//批量插入数据
var addRange = function (addObjs, tableName, callBack) {
    getConnection(function (connection) {
        var ps = new mssql.PreparedStatement(connection);
        var sql = "insert into " + tableName + "(";
        if (addObjs.length > 0) {
            for (var index in addObjs[0]) {
                if (typeof addObjs[0][index] == "number") {
                    ps.input(index, mssql.Int);
                } else if (typeof addObjs[0][index] == "string") {
                    ps.input(index, mssql.NVarChar);
                }
                sql += index + ",";
            }
        ...
        for (var j = 0; j < addObjs.length; j++) {
            var querydata = '';
            for (var index in addObjs[j]) {
                if (typeof addObjs[j][index] == "number") {
                    querydata += addObjs[j][index] + ",";
                } else if (typeof addObjs[j][index] == "string") {
                    querydata += "'" + addObjs[j][index] + "'" + ",";
                }
            }
            sql += '(' + querydata.substring(0, querydata.length - 1) + '),';
        }
        sql = sql.substring(0, sql.length - 1);
        ps.prepare(sql, function (err) {
            if (err)
                console.log(err);
            ps.execute(addObjs, function (err, recordset) {
                callBack(err, recordset);
                ps.unprepare(function (err) {
                    if (err)
                        console.log(err);
